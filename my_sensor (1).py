...
from 1_helper import EllipseFilamentWideSensorHelper
...
class HallFilamentWidthSensor:
    def __init__(self, config):
        ...
        self.sensor = EllipseFilamentWideSensorHelper(config)
    ...
    def update_filament_array(self, last_epos):
        ...
        if len(self.filament_array) > 0:
            ...
            if next_reading_position <= (last_epos + self.measurement_delay):
                self.filament_array.append([last_epos + self.measurement_delay, self.sensor()])
                if self.is_log:
    ...
    def extrude_factor_update_event(self, eventtime):
        ...
        # Check runout
        self.runout_helper.note_filament_present(self.sensor.check_for_virtual_f_swich_sensor())
                ...
                if pending_position <= last_epos:
                    ...
                    if (self.use_current_dia_while_delay
                        and (self.firstExtruderUpdatePosition
                             == pending_position)):
                        self.filament_width = self.sensor()
